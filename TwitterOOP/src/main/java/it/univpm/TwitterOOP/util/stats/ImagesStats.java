...
/**
 * Questa classe viene utilizzata per effettuare le statistiche sulle immagini considerando
 * la dimensione, l'altezza e la lunghezza
 * 
...
public class ImagesStats {
	...
	public void checkMinAndMaxHeight(Image img) {
		if(img.getSize().get(0).getHeight() < heightMin || heightMin ==0) heightMin = img.getSize().get(0).getHeight();
		if(img.getSize().get(0).getHeight() > heightMax) heightMax = img.getSize().get(0).getHeight();
	}
	...
	public void checkMinAndMaxWidth(Image img) {
		if(img.getSize().get(0).getWidth() < widthMin || widthMin == 0) widthMin = img.getSize().get(0).getWidth();
		if(img.getSize().get(0).getWidth() > widthMax) widthMax = img.getSize().get(0).getWidth();
	}
	...
	public void checkMinAndMaxDim(Image img) {
		if(img.getSize().get(0).getSize() < dimMin || dimMin ==0) dimMin = img.getSize().get(0).getSize();
		if(img.getSize().get(0).getSize() > dimMax) dimMax = img.getSize().get(0).getSize();
	}
	
	/**
	 * 
	 * @param counter le statistiche relative alle immagini considerando
	 * dimensione, lunghezza e altezza
	 */
